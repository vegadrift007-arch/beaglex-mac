# RamKiller — Design Spec

**Date**: 2026-04-30
**Author**: a77 (with Claude)
**Status**: Draft → awaiting user review
**Bundle ID**: `com.vannaq.ramkiller`
...
RamKiller 是一款 macOS 菜单栏 + 主窗口应用，整合 RAM 实时监控、进程管理、自动化告警、缓存清理、大文件查找、应用卸载、启动项管理等功能。目标是替代日常用户手工跑 `vm_stat` / `ps` / `sudo purge` 的工作流，并提供 CleanMyMac 风格的可视化交互。仅用于自用，**不上架 App Store**，因此可使用沙盒禁止的特权 API（`sudo purge`、跨进程 `kill`、清理用户缓存等）。
...
| 名称 / Bundle ID | RamKiller / `com.vannaq.ramkiller` |
...
```
RamKiller.xcodeproj/
├── RamKiller/                         (Target 1: 主 app)
│   ├── App/
│   │   ├── RamKillerApp.swift        @main, MenuBarExtra + WindowGroup
...
│   └── Info.plist
│
├── RamKillerHelper/                   (Target 2: 特权 helper)
...
- `RamKillerApp.swift` 实现 `MenuBarExtra` + `WindowGroup`
